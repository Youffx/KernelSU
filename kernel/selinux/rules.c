...
// Kernel < 5.10: selinux_state.ss is a pointer containing embedded policydb.
// No struct selinux_policy, no policy_mutex. Use rwlock_t for locking.
...
void apply_kernelsu_rules(void)
{
    ...
    write_lock_irqsave(&selinux_state.ss->policy_rwlock, flags);
    ...
    write_unlock_irqrestore(&selinux_state.ss->policy_rwlock, flags);
    reset_avc_cache();
}
...
static int apply_one_sepolicy_cmd(struct policydb *db, const struct sepol_data *header, const char **args)
{
    ...
    case KSU_SEPOLICY_CMD_TYPE_ATTR:
        ...
        if (!success) pr_err("sepol: %d failed.\n", header->cmd);
        return success ? 0 : -EINVAL;

    case KSU_SEPOLICY_CMD_ATTR:
        ret = sepol_require_not_all(args[0], "attribute");
        if (ret < 0) return ret;
        if (!ksu_attribute(db, args[0])) pr_err("sepol: %d failed.\n", header->cmd);
    ...
    case KSU_SEPOLICY_CMD_GENFSCON:
        ...
        if (ret < 0) return ret;
        if (!ksu_genfscon(db, args[0], args[1], args[2])) pr_err("sepol: %d failed.\n", header->cmd);
